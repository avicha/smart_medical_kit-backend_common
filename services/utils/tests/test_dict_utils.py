...
def test_dict_pick(util):
    source_dict = {'x': 1, 'y': 2, 'z': 3}
    target_dict = util.pick(source_dict, 'x', 'y')
    assert target_dict.has_key('x')
    assert target_dict.has_key('y')
    assert not target_dict.has_key('z')


def test_dict_pick_allow_field_not_exists(util):
    source_dict = {'x': 1, 'y': 2, 'z': 3}
    target_dict = util.pick(source_dict, 'x', 'y', 'u', 'v')
    assert target_dict.has_key('x')
    assert target_dict.has_key('y')
...
def test_dict_pick_not_allow_field_not_exists(util):
    source_dict = {'x': 1, 'y': None, 'z': 3}
    target_dict = util.pick(source_dict, 'x', 'y', 'u', 'v', allow_field_not_exists=False)
    assert target_dict.has_key('x')
    assert target_dict.has_key('y')
